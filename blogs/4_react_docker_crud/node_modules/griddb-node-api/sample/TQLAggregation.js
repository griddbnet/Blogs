...
var containerName = 'Sample_TQLAggregation';
...
store.dropContainer(containerName)
    ...
    .then(cont => {
        ...
        for (var i = 0;i < rowCount; i++) {
            ...
            console.log("Sample data generation: row=(%d, %s, %d)", i, nameList[i], numberList[i]);
        }
    ...
    .then((rs) => {
        while(rs.hasNext()) {
            aggregation = rs.next();
            max = aggregation.get(griddb.Type.LONG);
            console.log("TQL result: max=%d", max);
        }
        console.log('Success!');
        return true;
    })
